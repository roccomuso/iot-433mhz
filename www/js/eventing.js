...
events.on('newCardClick', function (code) {
  ...
  templating.renderTemplate('newCardForm.mustache', $('#main_modal_box'), view).then(function () {
    ...
    $cardForm.submit(function (e) {
      ...
      } else if ($type_selected == 'alarm') { form.append('trigger_code', $('#code option:selected').val()) }
...
events.on('renderInitCards', function (initData) {
  ...
  templating.renderTemplate('cards.mustache', $('#mainBody'), view).then(function () {
    ...
    } else { $('#cards_container').mixItUp({animation: { animateResizeContainer: false}}) }
...
events.on('uiMuteStatus', function (data) {
  ...
  if (data.notification_sound) { $dropdown_entry.html('<i class="fa fa-volume-off fa-fw"></i> Mute') } else { $dropdown_entry.html('<i class="fa fa-volume-up fa-fw"></i> Un-mute') }
})
...
events.on('uiArmStatus', function (data) {
  ...
  if (data.is_armed) { $dropdown_entry.html('<i class="fa fa-times-circle fa-fw"></i> Disarm') } else { $dropdown_entry.html('<i class="fa fa-rocket fa-fw"></i> Arm') }
})
...
function distinctElementsArray (array) { // O(n^2)
  ...
  array.forEach(function (elem) {
   		if (unique.indexOf(elem) === -1) { unique.push(elem) }
  })
  return unique
}
